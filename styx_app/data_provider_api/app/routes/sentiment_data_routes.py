...
from ..models import SentimentInferenceResultBatch, NewsIDs
from ..services.sentiment_data_services import (
...
@router.post("/sentiment_mark_processed")
...
@router.post("/sentiment_save_results")
async def save_sentiment_inference_results(
    sentiment_results: SentimentInferenceResultBatch,
    db: Session = Depends(get_db_session),
):
    try:
        successfully_saved_ids = save_sentiment_results(sentiment_results, db)
        if successfully_saved_ids:
            ...
            logger.warning("Failed to save Sentiment results")
            return JSONResponse(
                status_code=400, content={"detail": "Failed to save Sentiment results"}
            )
    except SQLAlchemyError as e:
        logger.error(f"SQLAlchemy error during saving Sentiment results: {str(e)}")
        return JSONResponse(
            status_code=500, content={"detail": "Internal server error"}
        )
    except Exception as e:
        logger.error(f"Unexpected error during saving Sentiment results: {str(e)}")
        return JSONResponse(
            status_code=500, content={"detail": "Internal server error"}
        )
